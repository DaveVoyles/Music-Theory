...
**Companion ADRs (proposed):** 0001 PixiJS dual-view · 0002 SongAnalyzerProvider · 0003 Phased mock analyzer
...
- **Human must never commit LLM API keys** into the static client; live analysis uses user/gateway URL only (called out in ADR 0002).
...
1. **ADR 0001 — PixiJS for both interactive views**  
   2D GPU diagrams; avoid Three.js 3D overhead for CoF + fretboard.

2. **ADR 0002 — SongAnalyzerProvider + domain DTO**  
   UI depends on a domain analysis contract, not a vendor chat API. Mock vs Http swap via config; **no secrets in the static bundle**.

3. **ADR 0003 — Phased analyzer: fixtures before live gateway**  
   v1 proves timeline → store → visuals with fixtures; live gateway is v1.1.

## Plan-Lenses Findings (advisory)
...
| security | low | Fixture song metadata is public domain/demo data only — no copyrighted full transcriptions claimed as official. |
...
| ux | low | Phone is best-effort; set an honest min-width and stack panels rather than claiming full mobile parity. |
...
## Glossary terms to capture

| Term | Definition |
|------|------------|
| Theory store | Zustand single source of truth for key, mode, minor form, focus degree, and analysis-driven jumps |
| Minor form | Natural, harmonic, or melodic minor variant affecting scale degrees 6 and/or 7 |
| Focus degree | Optional roman-numeral filter (I–vii°) restricting fretboard chord-tone emphasis |
| Power-chord emphasis | Stronger visual weight on Low E, A, and D strings for root-fifth shapes |
| SongAnalyzerProvider | Swappable analysis backend (Mock / Http) returning a domain song-analysis DTO |
| Dual-ring CoF | Circle with major keys on the outer ring and relative minors on the inner ring |

## Execution note

Planner **stops at approved doc + issues + hand-off**. Implementation is a separate orchestrator session per deliverable issues.
